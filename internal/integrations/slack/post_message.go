...
func (c *SlackHandler) PostMessage(
	ctx context.Context,
	...
	if !ok || text == "" {
		return &handler.ActionResult{
			StatusCode: http.StatusBadRequest,
			Error:      fmt.Errorf("missing or invalid message text"),
		}, nil
	}

	// Optional params
	var threadTs string
	if ts, ok := inputs["thread_ts"].(string); ok && ts != "" {
		threadTs = ts
	}
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		"https://slack.com/api/chat.postMessage", bytes.NewBuffer(body))
	if err != nil {
		return &handler.ActionResult{
			StatusCode: http.StatusInternalServerError,
