...
import (
	...
	"github.com/rkumar-bengaluru/Integrations/v2/internal/integrations/commons" // swap with slack, jira, etc.
	"github.com/rkumar-bengaluru/Integrations/v2/internal/integrations/ggmail"
...
func main() {
	...
	integration, err := ggmail.CreateGmailIntegration(ctx, database, logger, encryptionSvc, credentialRepo, tenantUID)
	if err != nil {
		logger.Fatal("error creating integration", zap.Error(err))
	}
	handlerInstance := ggmail.NewGmailHandler(encryptionSvc, bindingSvc, logger)
