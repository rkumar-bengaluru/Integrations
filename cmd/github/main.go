...
import (
	...
	"github.com/rkumar-bengaluru/Integrations/v2/internal/integrations/github" // swap with slack, jira, etc.
...
func main() {
	...
	integration, err := github.CreateGithubIntegration(ctx, database, logger, encryptionSvc, credentialRepo, tenantUID)
	if err != nil {
		logger.Fatal("error creating integration", zap.Error(err))
	}
	handlerInstance := github.NewGitHubHandler(encryptionSvc, bindingSvc, logger)
